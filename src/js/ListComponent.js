...
const prodlistComponent = Vue.component("prodlist", {
  ...
  methods: {
    ...
    AddtocartMethod: function (newItem) {
      ...
      let itemExist = this.cart.find(
        (cartItem) => cartItem.prodId === newItem.prodId
      );
      if (itemExist != undefined) {
        // если такой товар уже есть в корзине
        if (itemExist.prodQuantity + newItem.prodQuantity > 0) {
          //если результат добавления положительный
          itemExist.prodQuantity += newItem.prodQuantity;
        } else {
          //если результат добавления не положительный
          this.DeleteCartItem(newItem.prodId);
        }
      } else if (newItem.prodQuantity > 0) {
        //новый товар с положительным кол-вом
    ...
    ShowCartMethod: function () {
      ...
      for (let i = 0; i < this.cart.length; i++) {
        strToShow +=
          this.cart[i].prodQuantity + "шт. " + this.cart[i].prodId + "\n";
      }
